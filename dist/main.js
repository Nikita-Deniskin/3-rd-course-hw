...
const getCardsArray = (difficult) => {
  ...
  const shuffledCards = lodash__WEBPACK_IMPORTED_MODULE_0___default().shuffle(cards);
  const slicedArray = shuffledCards.slice(0, difficult / 2);
  const duplicatedArray = lodash__WEBPACK_IMPORTED_MODULE_0___default().concat(slicedArray, slicedArray);
  return lodash__WEBPACK_IMPORTED_MODULE_0___default().shuffle(duplicatedArray);
};
...
function renderGameComponent(level) {
  const cards = (0,_utils_js__WEBPACK_IMPORTED_MODULE_0__.getCardsArray)(level);
